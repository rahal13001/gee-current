...
| Tahap 3 T3-014 executor preparation | `PASS_WITH_NOTES` | `outputs/evidence/stage_3/T3-014_executor_preparation.result.txt`; executor membangun plan, menyiapkan/validasi inventory, membentuk subset AOI/depth/version/part, memakai batas waktu monthly scalar, dan dry-run 132/132 lulus; download aktual belum selesai dan T3-014 tetap `NOT_STARTED` |
...
20. T3-014 executor preparation `PASS_WITH_NOTES`: `python/03_download_glorys.py` membangun plan lokal, melakukan dry-run fail-closed, membentuk parameter subset dengan AOI regional, depth `0.494025`, version/part `202311/default`, memakai batas scalar untuk monthly dan timestamp terakhir `00:00:00` untuk daily JFM, melakukan validasi inventory secara plan-scoped, dan hanya memproses plan aktif tanpa menimpa status batch lain. Mode aktual memerlukan `--execute`; recovery permanent failure memerlukan `--job-id --force-after-quarantine`; batch belum selesai sehingga T3-014 tetap `NOT_STARTED`.
21. T3-016 reconciliation implementation evidence tersedia melalui `python/05_reconcile_inventory.py` dan `outputs/evidence/stage_3/T3-016_inventory_reconciliation.result.txt`: pemeriksaan read-only aktual menemukan 165 job, 165 file aktif, 1125 expected timesteps, 165 checksum cocok, 0 partial, dan 3 quarantine artifact sebagai note. Status field T3-014 sampai T3-017 tidak diubah tanpa change-control eksplisit.
22. AOI aktif diterapkan menjadi `eastern_indonesia_regional_001` dengan user-provided bbox `N=4.265137, W=122.986190, S=-12.191592, E=143.326183`; `pilot_001` tetap terpisah untuk baseline T2, sedangkan polygon/water mask dan download operasional belum dijalankan.
23. Audit kontrol GitHub remote dan review ADR yang masih `PROPOSED` tetap diperlukan sebelum release.
