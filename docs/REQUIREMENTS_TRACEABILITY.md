...
Baseline: 2026-08-02. Status follows the project vocabulary; `PASS` is not used here.
Evidence is local/offline unless explicitly stated otherwise.
...
| FND-007..FND-010 | Setup, user auth boundary, governance, Cloud monitoring | M0 | `outputs/evidence/foundation/FND-007_setup_report.md`, `FND-009_governance_record.md`, `FND-010_cost_monitoring_plan.md` | — | BLOCKED | foundation evidence | actual Project ID/tier/auth require user and approval |
...
| FND-019 | GitHub push protection/secret scanning review | M0 | `outputs/evidence/foundation/FND-019_github_security_review.md` | — | BLOCKED | review record | no `.git`/remote available |
...
| FND-SKILL-002 | Verifikasi GEEMu | M0 | `docs/audits/TOOLS_AND_SKILLS_INVENTORY.md`, `GEEMU_FOUNDATION_RUN.md` | local import/reference check | TESTED_WITH_NOTES | skill/reference/template inventory | runtime/auth blocked and intentionally not run |
...
| GOV-02 | Project ID khusus | M0 | — | TST-GOV-002 | BLOCKED | — | OD-002 |
| GOV-03 | Tier nonkomersial | M0 | — | TST-GOV-003 | BLOCKED | — | OD-003 |
...
The active document paths are the files currently present under `docs/`; the expected `docs/stages/`,
`docs/adr/`, `IMPLEMENTATION_STATUS.md`, and `REQUIREMENTS_TRACEABILITY.md` gaps were identified by audit,
and the Foundation artifacts now created are registered above. Stage 0–3 documents remain normative inputs.
