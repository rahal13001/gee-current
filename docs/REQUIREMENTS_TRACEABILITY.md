...
| FR-META-04 | Dataset version/part | T0 | `outputs/evidence/stage_0/metadata_snapshot_2026-08-03.json`, `python/common/depth_metadata.py` | `outputs/evidence/stage_0/T0_depth_validator.result.txt` | PASS_WITH_NOTES | Metadata version/part `202311/default` and 50 active depth levels recorded; top model layer `0.49402499198913574 m` matches `0.494025 m` within tolerance; descending positive-down order is explicit | User-managed active evidence; no NetCDF validation was performed |
| FR-META-05 | Stop on material metadata change | T0 | `python/common/metadata_guard.py` | `outputs/evidence/stage_0/T0_metadata_guard.result.txt` | PASS_WITH_NOTES | Synthetic changed-dataset case fails closed; sanitized 2026-08-02 to 2026-08-03 comparison exits 0 | Comparison is user-managed snapshot evidence; raw live response and NetCDF remain outside Codex scope |
| T2-001..T2-003 | AOI pilot, metadata preflight, and dry-run | T2 | `config/study_area.json`, `config/pilot_config.example.json`, `tools/prepare_t2_pilot_plan.ps1` | `outputs/evidence/stage_2/T2-002_003_pilot_preflight_and_dry_run.result.txt` | PASS_WITH_NOTES | User-defined AOI and approved metadata generate exactly 29 daily dates for 2020-02-01 through 2020-03-01 exclusive | Plan was offline-only; NetCDF core validation is recorded separately; GeoTIFF, asset access, upload, and operational computation remain pending |
| T2-004..T2-011 | Pilot NetCDF download and core validation | T2 | User-managed Copernicus subset; local xarray/h5netcdf read-only validation | `outputs/evidence/stage_2/T2-004_011_netcdf_validation.result.txt` | PASS_WITH_NOTES | Retry file opened successfully; 29 timestamps, `uo`/`vo`, units, top depth, monotonic grid, mask presence, CF packing reconstruction, and finite ranges were checked | Initial request produced 30 timestamps and was retained; corrected retry is the accepted candidate; GeoTIFF comparison, upload, and Earth Engine validation remain pending |
| T2-012 | GeoTIFF conversion | T2 | `tools/convert_t2_pilot_to_geotiff.py`, `requirements.txt` | `outputs/evidence/stage_2/T2-012_013_geotiff_validation.result.txt` | PASS_WITH_NOTES | 29 float32 two-band GeoTIFFs generated with EPSG:4326, NaN nodata, and no resampling | No Earth Engine upload or computation; exact polygon/water mask remains downstream |
| T2-013 | NetCDF–GeoTIFF comparison | T2 | `tools/validate_t2_geotiff_against_netcdf.py` | `outputs/evidence/stage_2/T2-012_013_geotiff_validation.result.txt` | PASS_WITH_NOTES | 29/29 files, `uo`/`vo`, CRS, mask, timestamps, and values matched; max absolute difference `2.98e-08` | Upload and Earth Engine validation remain pending |
...
Current reconciliation note: Foundation artifacts and audit reports are local evidence; user-reported runtime/setup facts are not independent Cloud or credential audits. The T1 formula/statistics baseline is implemented and synthetically tested in `python/common/scientific_formulas.py` and `python/common/descriptive_statistics.py`; ddof and percentile method remain explicit open parameters, and FR-PY operational analytics remain downstream until validated data exist.
