...
| FR-META-01 | Describe produk/dataset | T0 | `tools/describe_copernicus_metadata.ps1`, `docs/audits/COPERNICUS_METADATA_READONLY_CHECK.md` | `outputs/evidence/stage_0/T0_describe_wrapper.plan.result.txt` | PASS_WITH_NOTES | Offline wrapper plan and prior read-only describe audit record the approved product and dataset IDs | Wrapper is plan-only by default; active describe was not rerun; no authentication or network |
...
| FR-META-04 | Dataset version/part | T0 | `outputs/evidence/stage_0/metadata_snapshot_2026-08-02.json`, `python/common/depth_metadata.py` | `outputs/evidence/stage_0/T0_depth_validator.result.txt` | IMPLEMENTED | Metadata version `202311` recorded; local depth validator fails closed when the full list is absent | Part remains unavailable; active 50-level extraction is still required |
| FR-META-05 | Stop on material metadata change | T0 | `python/common/metadata_guard.py` | `outputs/evidence/stage_0/T0_metadata_guard.result.txt` | TESTED | Synthetic changed-dataset case fails closed; approved snapshot comparison exits 0 | Live metadata comparison was not run; no material-change claim for the active service |
