...
if ($depth -and ($depth.label -ne 'top_model_layer' -or $depth.selection_method -ne 'exact_after_verification' -or [math]::Abs([double]$depth.tolerance_m - 0.000001) -gt 0.000000000001 -or $depth.full_50_level_extraction_status -ne 'NOT_RUN')) { $errors.Add('depth_selection:approved_metadata_mismatch') }
if ($stats -and $stats.speed_thresholds_mps.Count -ne 0) { $errors.Add('statistics:thresholds_must_remain_empty_until_approved') }
if ($stats -and ($stats.direction_convention -ne 'towards_clockwise_from_north' -or $stats.threshold_status -ne 'TBD_NOT_INVENTED' -or $null -ne $stats.minimum_valid_percentage)) { $errors.Add('statistics:convention_or_open_parameter_mismatch') }
...
Write-Output 'AOI ordering, EPSG:4326, period/JFM fields, depth selection metadata, statistic fields, dataset IDs, timezone, asset-root prefix, and open thresholds validated.'
