...
if ($stats -and ($stats.direction_convention -ne 'towards_clockwise_from_north' -or $stats.threshold_status -ne 'RESOLVED_GLOBAL_AOI_P90' -or [double]$stats.minimum_valid_area_fraction -ne 0.95 -or $stats.threshold_method -ne 'relative_high_current_threshold_global_p90')) { $errors.Add('statistics:convention_or_resolved_decision_mismatch') }
